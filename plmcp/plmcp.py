...
from .util import filter_result_dataframe, dot_product, read_fasta, embedding_load, embedding_cos_similarity, make_parent_dir, draw_alignment, read_CP_fasta,read_CP_target_fasta,read_query_fasta_to_files,read_target_fasta_to_files
...
#            print(f"The two proteins maybe evolved by circular permutation\n,check2")
def plmCP_fisher_pipelineWithAAidOnly( query_AA_id ,target_AA_id, mode = 'local',query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
#def plmCP_fisher_pipeline(query_fasta, target_fasta, mode = 'global', query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
    method = 'plmcp'
    print(f"Align with method: {method}")
    ...
    query_sequence = ''.join([line[0] for line in QueryAAid])
    target_sequence = ''.join([line[0] for line in TargetAAid])
    print('queryName',query_AA_id[-9:],'TargetName',target_AA_id[-9:])
    #print('target seq',target_sequence)
    print('query seq',query_sequence)
    print('target seq',target_sequence)
    #query_sequences,query_CP_sequences = read_query_fasta_to_files(query_sequence)	
    #target_sequences,target_CP_sequences = read_target_fasta_to_files(target_sequence)		
    _,_  = read_query_fasta_to_files(query_sequence)	
    _,_ = read_target_fasta_to_files(target_sequence)	
    query_fasta= "example/query_output.fasta"
    target_fasta= "example/target_output.fasta"    

    #query_CP_fasta= "example/CP_query_output.fasta"
    #target_CP_fasta= "example/CP_target_output.fasta"    
    _, query_sequences = read_fasta(query_fasta)
    _, target_sequences = read_fasta(target_fasta)
    _, query_CP_sequences = read_CP_fasta(query_fasta)
