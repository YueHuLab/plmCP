...
def pairwise_align(embedding1, embedding2, seq1, seq2, mode, method = 'plmcp'):
    ...
    #print(seq2)
    #print("\n")
    #resultIndex= [(c1+1,c2+1) for c1, c2 in row.indices]
    #resultIndex= [[c1+1, c2+1] for c1, c2 in row.indices]


    return row['score'].item(), aln,  row['alnlen'].item(),row.indices,row['scoreAll']
    #return row['score'].item(), aln,  row['alnlen'].item(),resultIndex,row['scoreAll']
        ...
        for single_query in tqdm(query_sequences, desc="Query"):
            for single_target in target_sequences:
                ...
                if if_stdout:
                    ...
                    print(f"{single_query}\t{single_target}AlignIndex\n{rowIndex}\n")
            ...
            for single_CP_target in target_CP_sequences:
                ...
                if if_stdout:
                    print(f"singe query with dumplicated target \n")
                    print(f"{single_query}\t{single_CP_target}\t Score = {score_tCP1}\n")
                    print(f"{single_query}\t{single_CP_target}\n{results_tCP1}\n")
                    print(f"{single_query}\t{single_CP_target}\t Len={alnlen_tCP1}\n")
                    #resultIndex= [[c1+1, c2+1] for c1, c2 in rowIndex_tCP1]
                    resultIndex= []
                    for c1, c2 in rowIndex_tCP1:
                        if (c1+1) > (len(query_sequences[single_query])):
                            c1=c1-int(len(query_sequences[single_query]))+1
                        else:
                            c1=c1+1
                        if (c2+1) > (len(target_CP_sequences[single_CP_target])/2.0):
                            c2=c2-int(len(target_CP_sequences[single_CP_target])/2.0)+1
                        else:
                            c2=c2+1
                        resultIndex.append((c1,c2))
                    print(f"{single_query}\t{single_CP_target}AlignIndex\n{resultIndex}\n")
                    print(f"{single_query}\t{single_CP_target}AlignIndex\n{rowIndex_tCP1}\n")
                    #print(f"{single_query}\t{single_target}\t ScoreAll = {scoreAll}\n")
                if (output_path != None):
                    f1.write(f"{single_query}\t{single_CP_target}\t{score_tCP1}\n")
                    f2.write(f"{single_query}\t{single_CP_target}\n{results_tCP1}\n\n")
                    protein_pair_dict[single_query].append((single_target, score))
    ...
    if (search_result_setting == None):
        for single_CP_query in tqdm(query_CP_sequences, desc="Query"):
            for single_target in target_sequences:
                ...
                if if_stdout:
                    print(f"dumplicated query with single target\n")
                    ...
                    print(f"{single_CP_query}\t{single_target}AlignIndex\n{rowIndexCP}\n")
                    resultIndexCP= []
                    for c1, c2 in rowIndexCP:
                        if (c1+1) > (len(query_CP_sequences[single_CP_query])/2.0):
                            c1=c1-int(len(query_CP_sequences[single_CP_query])/2.0)+1
                        else:
                            c1=c1+1
                        if (c2+1) > (len(target_sequences[single_target])):
                            c2=c2-int(len(target_sequences[single_target]))+1
                        else:
                            c2=c2+1
                        resultIndexCP.append((c1,c2))
                    print(f"{single_query}\t{single_CP_target}AlignIndex\n{resultIndexCP}\n")
...
########################################
    ...
    #    for indexCP in LISTCProwIndex:
        ...
        # check if gap occur
            ...
            if c1 == c1_prev:
                A_no_relation_one=A_no_relation_one+1
            if c2 == c2_prev:
                A_no_relation_two=A_no_relation_one+1
	...
	# merge into 3 line string
        ...
        # check if gap occur
            ...
            if c1 == c1_prev:
                B_no_relation_one=B_no_relation_one+1
            if c2 == c2_prev:
                B_no_relation_two=B_no_relation_one+1
            c1_prev = c1
            c2_prev = c2
	# merge into 3 line string
...
#        print(f"Test between  {LISTquery[i]} \t {LISTtarget[i]}\n")
 ...
 #           print(f"The two proteins maybe evolved by circular permutation\n")
        ...
        #LISTresults.append(results)
        #LISTalnlen[i]
        CP_check_A=A/len(LISTquery_seq[i])
        CP_check_AN1=A_no_relation_one/len(LISTquery_seq[i])
        CP_check_AN2=A_no_relation_two/len(LISTquery_seq[i])
        CP_check_B=B/len(LISTtarget_seq[i])
        CP_check_BN1=B_no_relation_one/len(LISTtarget_seq[i])
        CP_check_BN2=B_no_relation_two/len(LISTtarget_seq[i])
...
#        print(f"resFIsher CPcheck2 {CPcheck2}\n")
        print(f"TestBetween \t {LISTquery[i]} \t {LISTtarget[i]}\t queryLen \t{len(LISTquery_seq[i])}\t targetLen \t{len(LISTtarget_seq[i])}\t CP1 \t {CPcheck}\t CP2 \t{CPcheck2}\t info \t { LISTscore[i]}\t {LISTalnlen[i]}\t {LISTscore_tCP1[i]} \t {LISTalnlen_tCP1[i]} \t {LISTCPscore[i]} \t {LISTCPalnlen[i]} \t {LISTCPscore_tCP2[i]} \t {LISTCPalnlen_tCP2[i]} \t CP_check_A \t {CP_check_A} \t CP_check_AN1 \t {CP_check_AN1} \t CP_check_AN2 \t {CP_check_AN2} \tCP_check_B \t {CP_check_B} \t CP_check_BN1 \t {CP_check_BN1} \t CP_check_BN2 \t {CP_check_BN2}  \n")
#        if (score<CPscore and alnlen < alnlen_tCP1 and CPcheck2 < 0.5):
#            print(f"The two proteins maybe evolved by circular permutation\n,check2")
def plmCP_fisher_pipelineWithAAid(query_fasta, target_fasta, query_AA_id ,target_AA_id, mode = 'local',query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
#def plmCP_fisher_pipeline(query_fasta, target_fasta, mode = 'global', query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
    method = 'plmcp'
    print(f"Align with method: {method}")
    ...
    TargetAAid=[]
    if (query_AA_id != None):
        f_queryAA = open(query_AA_id, "r")
        lines = f_queryAA.readlines()
        for line in lines:
                QueryAAid.append(line)
    if (target_AA_id != None):
        f_targetAA = open(target_AA_id, "r")
        lines = f_targetAA.readlines()
        for line in lines:
                TargetAAid.append(line)
    ...
    if (search_result_setting == None):
        for single_query in tqdm(query_sequences, desc="Query"):
            for single_target in target_sequences:
                ...
                LISTtarget_seq.append(target_sequences[single_target])
                #print(f"query \t query {single_query}\n")
                assert len(QueryAAid)== len(query_sequences[single_query]), 'two list QueryAAid are not equl'
                assert len(TargetAAid)== len(target_sequences[single_target]), 'two list TargetAAid are not equl'
                if if_stdout:
                    print(f"{single_query}\t{single_target}\t Score = {score}\n")
                    print(f"{single_query}\t{single_target}\n{results}\n")
                    print(f"{single_query}\t{single_target}\t Len={alnlen}\n")
                    #print(f"{single_query}\t{single_target}AlignIndex\n{rowIndex}\n")
                    resultIndex_single= []
                    for c1, c2 in rowIndex:
                        resultIndex_single.append((QueryAAid[c1],TargetAAid[c2]))
                    print(f"{single_query}\t{single_target}AA_id\n{resultIndex_single}\n")
                    #print(f"{single_query}\t{single_target}\t ScoreAll = {scoreAll}\n")
                if (output_path != None):
                    f1.write(f"{single_query}\t{single_target}\t{score}\n")
                    f2.write(f"{single_query}\t{single_target}\n{results}\n\n")
            ...
            for single_CP_target in target_CP_sequences:
                ...
                if if_stdout:
                    ...
                    for c1, c2 in rowIndex_tCP1:
                        if (c1+1) > (len(query_sequences[single_query])):
                            #c1=c1-int(len(query_sequences[single_query]))+1
                            c1=c1-int(len(query_sequences[single_query]))
                        else:
                            c1=c1
                            #c1=c1+1
                        if (c2+1) > (len(target_CP_sequences[single_CP_target])/2.0):
                            #c2=c2-int(len(target_CP_sequences[single_CP_target])/2.0)+1
                            c2=c2-int(len(target_CP_sequences[single_CP_target])/2.0)
                        else:
                            #c2=c2+1
                            c2=c2
                        resultIndex.append((QueryAAid[c1],TargetAAid[c2]))
                    print(f"{single_query}\t{single_CP_target}AlignIndex\n{resultIndex}\n")
                    #print(f"{single_query}\t{single_CP_target}AlignIndex\n{rowIndex_tCP1}\n")
                    #print(f"{single_query}\t{single_target}\t ScoreAll = {scoreAll}\n")
                if (output_path != None):
            ...
            for single_target in target_sequences:
                ...
                if if_stdout:
                    ...
                    for c1, c2 in rowIndexCP:
                        if (c1+1) > (len(query_CP_sequences[single_CP_query])/2.0):
                            #c1=c1-int(len(query_CP_sequences[single_CP_query])/2.0)+1
                            c1=c1-int(len(query_CP_sequences[single_CP_query])/2.0)
                        else:
                            #c1=c1+1
                            c1=c1
                        if (c2+1) > (len(target_sequences[single_target])):
                            #c2=c2-int(len(target_sequences[single_target]))+1
                            c2=c2-int(len(target_sequences[single_target]))
                        else:
                            #c2=c2+1
                            c2=c2
                        resultIndexCP.append((QueryAAid[c1],TargetAAid[c2]))
                    print(f"{single_query}\t{single_CP_target}AlignIndex\n{resultIndexCP}\n")
                    #print(f"{single_CP_query}\t{single_target}\t ScoreAll ={scoreAllCP}\n")
                if (output_path != None):
                    fCP1.write(f"{single_CP_query}\t{single_target}\t{CPscore}\n")
...
#        if (score<CPscore and alnlen < alnlen_tCP1 and CPcheck2 < 0.5):
#            print(f"The two proteins maybe evolved by circular permutation\n,check2")
def plmCP_fisher_pipelineWithAAidReverse(target_fasta,query_fasta, target_AA_id, query_AA_id, mode = 'local',query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
#def plmCP_fisher_pipeline(query_fasta, target_fasta, mode = 'global', query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
    method = 'plmcp'
    print(f"Align with method: {method}")
...
# Set precision level for Decimal calculations
getcontext().prec = 50

from scipy.special import erfc


def calculate_e_value(S_total, sigma, N, m, n):
    # Step 1: Standardize the cumulative score to a standard normal Z score with high precision
    S_total = Decimal(S_total)
    sigma = Decimal(sigma)
    N = Decimal(N)

    Z_total = S_total / (Decimal(math.sqrt(N)) * sigma)
    
    # Step 2: Calculate the P-value using the complementary error function for more precision
    # Use Decimal for input, but the erfc function is from scipy, so we'll convert back and forth
    if Z_total > 6:  # For very large Z-scores, use erfc for better precision
        # erfc(x) gives the probability of being greater than x for a standard normal distribution
        #P_value = Decimal(0.5) * Decimal(erfc(float(Z_total / Decimal(math.sqrt(2)))))
        #    if Z_total > 6:  # For Z_total greater than 6, set P_value to a very small number
        P_value = Decimal('1e-50')
    else:
        # If Z_total is not extremely large, using norm.cdf is fine
        P_value = Decimal(1) - Decimal(norm.cdf(float(Z_total)))
    
    # Step 3: Calculate the E-value with high precision
    m = Decimal(m)
    n = Decimal(n)
    E_value = P_value * m * n
    print("Evalue ",S_total,sigma,N,Z_total,P_value,m,n,E_value)    
    return E_value




def calculate_e_value_t1(S_total, sigma, N, m, n):
    # Step 1: Standardize the cumulative score to a standard normal Z score with high precision
    S_total = Decimal(S_total)
    sigma = Decimal(sigma)
    N = Decimal(N)
    ...
    # Step 2: Calculate the P-value using the CDF of the standard normal distribution
    # Use Decimal for input, but the CDF function is from scipy, so we'll convert back and forth
    P_value = Decimal(1) - Decimal(norm.cdf(float(Z_total)))
...
def plmCP_fisher_pipelineWithAAidEvalue(query_fasta, target_fasta, query_AA_id ,target_AA_id, mode = 'local',query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
#def plmCP_fisher_pipeline(query_fasta, target_fasta, mode = 'global', query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
    method = 'plmcp'
    print(f"Align with method: {method}")
        ...
        for single_query in tqdm(query_sequences, desc="Query"):
            for single_target in target_sequences:
                ...
                if if_stdout:
                    ...
                    print(f"{single_query}\t{single_target}\n{results}\n")
                    print(f"{single_query}\t{single_target}\t Len={alnlen}\n")
                    Evalue=calculate_e_value(score, 1.0, alnlen, len(QueryAAid), len(TargetAAid))
                    Evalue_float=float(Evalue)					
                    print(f"Evalue\t{Evalue_float:.2E}\n")
                    #print(f"{single_query}\t{single_target}AlignIndex\n{rowIndex}\n")
                    resultIndex_single= []
                    for c1, c2 in rowIndex:
                        resultIndex_single.append((QueryAAid[c1],TargetAAid[c2]))
            ...
            for single_CP_target in target_CP_sequences:
                ...
                if if_stdout:
                    ...
                    print(f"{single_query}\t{single_CP_target}\t Len={alnlen_tCP1}\n")
                    #resultIndex= [[c1+1, c2+1] for c1, c2 in rowIndex_tCP1]
                    Evalue_tCP1=calculate_e_value(score_tCP1, 1.0, alnlen_tCP1, len(QueryAAid), 2*len(TargetAAid))
                    print(f"Evalue targetCP\t{Evalue_tCP1:.2E}\n")					
                    resultIndex= []
                    for c1, c2 in rowIndex_tCP1:
                        if (c1+1) > (len(query_sequences[single_query])):
                            #c1=c1-int(len(query_sequences[single_query]))+1
        ...
        for single_CP_query in tqdm(query_CP_sequences, desc="Query"):
            for single_target in target_sequences:
                ...
                if if_stdout:
                    ...
                    print(f"{single_CP_query}\t{single_target}\n cpResult = \n{CPresults}\n")
                    print(f"{single_CP_query}\t{single_target}\t cpLen = {CPalnlen}\n")
                    CPEvalue=calculate_e_value(CPscore, 1.0, CPalnlen, 2*len(QueryAAid), len(TargetAAid))
                    print(f"Evalue queryCP\t{CPEvalue:.2E}\n")						
                    #print(f"{single_CP_query}\t{single_target}AlignIndex\n{rowIndexCP}\n")
                    resultIndexCP= []
                    for c1, c2 in rowIndexCP:
                        if (c1+1) > (len(query_CP_sequences[single_CP_query])/2.0):
