...
def plmcp_gather_all_paths(array: np.ndarray,
					norm: bool = True,
					mode: str = 'local',
					#gap_extension: float = 1.0,
					gap_extension: np.float32 =1.0  ,#huyue0422
	...
	#assert isinstance(gap_extension, (int, np.float32))
	...
	score_matrix,antecedent = fill_score_matrix(arraynorm, gap_extension = gap_extension, mode=mode)
	...
	path = traceback_from_point_opt2(score_matrix,antecedent, indice,gap_extension = gap_extension, mode=mode)
	if with_scores:
		return (path, score_matrix)
	else:
		return path

def plmcp_search_paths(submatrix: np.ndarray,
		 path: Tuple[list],
		 mode: str = 'local',
	...
	if any(spans):
		for idx, (start, stop) in enumerate(spans):
			...
			#huyue
                        #y_start, x_start = y[start:start], x[start:start]
                        #arr_values_start = submatrix[y_start, x_start]
			#huyue
                        #y_stop, x_stop = y[stop:stop], x[stop:stop]
                        #arr_values_stop = submatrix[y_stop, x_stop]
			...
			spans_locations[keyid] = {
				...
				'score': arr_values.max(),
				'scoreAll': arr_values,
				#'score': arr_values.sum(),
				#'score_start': arr_values_start.mean(),
				#'score_stop': arr_values_stop.mean(),
				"alnlen": alnlen,
				"mode": mode
			}
	if as_df:
