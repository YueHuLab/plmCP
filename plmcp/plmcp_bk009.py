...
class plmcp:
    ...
    MODE: str = 'local'
    #GAP_EXT: float = 1.0
    #GAP_EXT: float = 100.0 #huyue
    GAP_EXT: float = 1.0 #huyue
...
def pairwise_align(embedding1, embedding2, seq1, seq2, mode, method = 'plmcp'):
    ...
    return row['score'].item(), aln,  row['alnlen'].item(),row.indices

def plmCP_pipeline(query_fasta, target_fasta, mode = 'global', query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
    method = 'plmcp'
    ...
    if (search_result_setting == None):
        for single_query in tqdm(query_sequences, desc="Query"):
            for single_target in target_sequences:
                score, results,alnlen = pairwise_align(query_embeddings[single_query], target_embeddings[single_target], query_sequences[single_query], target_sequences[single_target], mode, method = method)
                if if_stdout:
...
########################################
    ...
    if (search_result_setting == None):
        for single_CP_query in tqdm(query_CP_sequences, desc="Query"):
            for single_target in target_sequences:
                CPscore, CPresults,CPalnlen = pairwise_align(query_CP_embeddings[single_CP_query], target_embeddings[single_target], query_CP_sequences[single_CP_query], target_sequences[single_target], mode, method = method)
                if if_stdout:
...
#def plmCP_fisher_pipeline(query_fasta, target_fasta, mode = 'global', query_embedding_path = None, target_embedding_path = None, search_result_setting = None, output_path = None, if_stdout = True):
    ...
    if (search_result_setting == None):
        for single_query in tqdm(query_sequences, desc="Query"):
            for single_target in target_sequences:
                score, results,alnlen,rowIndex = pairwise_align(query_embeddings[single_query], target_embeddings[single_target], query_sequences[single_query], target_sequences[single_target], mode, method = method)
                if if_stdout:
                    print(f"{single_query}\t{single_target}\t Score = {score}\n")
                    print(f"{single_query}\t{single_target}\n{results}\n")
                    print(f"{single_query}\t{single_target}\t Len={alnlen}\n")
            ...
            for single_CP_target in target_CP_sequences:
                score_tCP1, results_tCP1,alnlen_tCP1,rowIndex_tCP1 = pairwise_align(query_embeddings[single_query], target_CP_embeddings[single_CP_target], query_sequences[single_query], target_CP_sequences[single_CP_target], mode, method = method)
...
########################################
    ...
    if (search_result_setting == None):
        for single_CP_query in tqdm(query_CP_sequences, desc="Query"):
            for single_target in target_sequences:
                CPscore, CPresults,CPalnlen,rowIndexCP = pairwise_align(query_CP_embeddings[single_CP_query], target_embeddings[single_target], query_CP_sequences[single_CP_query], target_sequences[single_target], mode, method = method)
                if if_stdout:
                    print(f"{single_CP_query}\t{single_target}\t Score = {CPscore}\n")
                    print(f"{single_CP_query}\t{single_target}\n{CPresults}\n")
                    print(f"{single_CP_query}\t{single_target}\t Len={CPalnlen}\n")
                if (output_path != None):
                    ...
                    protein_CP_pair_dict[single_CP_query].append((single_target, CPscore))
            for single_CP_target in target_CP_sequences:
                CPscore_tCP2, CPresults_tCP2,CPalnlen_tCP2,rowIndex_tCP2 = pairwise_align(query_CP_embeddings[single_CP_query], target_CP_embeddings[single_CP_target], query_CP_sequences[single_CP_query], target_CP_sequences[single_CP_target], mode, method = method)
